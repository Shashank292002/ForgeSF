...
/// several subcommands: `sf apex run --json` exits **0** when Apex compiles
...
pub async fn run_sf_json(
    ...
    blocking(move || {
        ...
        // `apex run` carries its own success flag inside a status-0
...
/// Runs an anonymous Apex block.
///
/// The CLI dropped `apex execute`, so the old call failed with
/// "apex execute is not a sf command". The command is `apex run`, and it
/// takes the block from a file: with no flags it reads stdin interactively
/// until EOF, which is not something to depend on from a window.
#[tauri::command]
pub async fn run_apex(
    ...
    blocking(move || {
        let run = RunGuard::begin(run_id);
        let file = TempFile::create("apex", code.trim())?;

        let mut command = sf_command()?;
        command.args(["apex", "run", "--target-org", &username, "--json", "--file"]);
        ...
        let output = run_cancellable(command, None, &run)?;
        let json = parse_sf_json(&output)?;
        // Apex that compiles and then throws still exits 0; the failure is
        // inside the envelope.
        if json.pointer("/result/success") == Some(&serde_json::Value::Bool(false)) {
            return Err(apex_failure_message(&json).into());
        }
...
mod tests {
    ...
    /// `sf apex run --json` reports a runtime throw inside a status-0
