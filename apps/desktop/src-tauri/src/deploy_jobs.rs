...
use crate::changes;
use crate::commands::{
    blocking, lock, now_millis, package_directories, package_xml, parse_sf_json, read_registry,
    resolve_paths, run_with_limits, sf_command, sf_error_message, sf_plain_error,
    source_api_version, to_relative_string, workspace_root, write_atomic, RunGuard, TempFile,
    MAX_INLINE_METADATA_CHARS,
};
...
fn start_job(command: Command, run: &RunGuard) -> Result<String, String> {
    ...
    json.pointer("/result/id")
        ...
        .map(str::to_string)
        .ok_or_else(|| "The Salesforce CLI did not return a job id.".to_string())
}
...
pub async fn deploy_start(
    ...
    blocking(move || {
        ...
        let record = DeployRecord {
            job_id,
            username,
            workspace_name: workspace_name(&app, workspace_id.as_deref(), &workspace),
            workspace_id,
...
pub async fn deploy_quick_start(
    ...
    blocking(move || {
        if !valid_job_id(&validation_job_id) {
            return Err(format!("'{validation_job_id}' is not a deploy job id."));
        }
        let run = RunGuard::begin(run_id);
        let workspace = workspace_root(&app, workspace_id.as_deref())?;
        ...
        update_history_at(&path, |records| {
            ...
            let record = DeployRecord {
                ...
                workspace_name: validation
                    ...
                    .unwrap_or_else(|| workspace_name(&app, workspace_id.as_deref(), &workspace)),
...
pub async fn deploy_report(
    ...
    blocking(move || {
        if !valid_job_id(&job_id) {
            return Err(format!("'{job_id}' is not a deploy job id."));
        }
        ...
        let json: serde_json::Value =
            serde_json::from_str(stdout.trim()).map_err(|_| sf_plain_error(&output))?;
...
#[tauri::command]
pub async fn deploy_history(app: tauri::AppHandle) -> Result<Vec<DeployRecord>, String> {
    blocking(move || {
        let path = history_path(&app)?;
        let _guard = lock(&HISTORY_LOCK);
        let mut records = read_history_file(&path);
...
mod tests {
    ...
    fn scopes_become_source_dirs_metadata_or_a_manifest() {
        let root = std::env::temp_dir().join(format!(
            ...
            crate::commands::next_temp_suffix()
        ));
