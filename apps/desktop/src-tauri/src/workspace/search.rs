...
use crate::workspace::paths::{is_ignored_path, to_relative_string};
...
#[tauri::command]
pub async fn cancel_workspace_search() -> AppResult<()> {
...
pub async fn list_workspace_files(
    app: tauri::AppHandle,
    workspace_id: Option<String>,
) -> AppResult<WorkspaceFileList> {
    blocking(move || {
        let root = workspace_root(&app, workspace_id.as_deref())?;
        list_files(&root).map_err(AppError::from)
    })
    .await
}
