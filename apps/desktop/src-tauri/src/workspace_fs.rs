...
use crate::apex;
use crate::commands::{
    blocking, next_temp_suffix, package_directories, read_editor_text, resolve_in_workspace,
    source_api_version, to_relative_string, workspace_root, write_atomic,
};
...
/// Starts the error a save returns when the file changed on disk after it was
/// read. The editor recognises it and asks what to do.
pub(crate) const CHANGED_ON_DISK: &str = "CHANGED_ON_DISK";

/// Used for new Apex when the project names no `sourceApiVersion`.
const DEFAULT_API_VERSION: &str = "65.0";
...
pub async fn read_workspace_file(
    ...
    blocking(move || {
        ...
        Err("The file kept changing while it was being read. Try again.".to_string())
    })
    .await
}
    ...
    blocking(move || {
        ...
        if let Some(expected) = expected {
            match stamp_of(&full_path) {
                ...
                Some(_) => {
                    return Err(format!(
                        "{CHANGED_ON_DISK}: '{path}' changed on disk after it was opened."
                    ))
                }
                None => {
                    return Err(format!(
                        "{CHANGED_ON_DISK}: '{path}' was deleted on disk after it was opened."
                    ))
                }
            }
        }
        ...
        stamp_of(&full_path).ok_or_else(|| "The file was saved but can't be read back.".to_string())
    })
    .await
}
...
pub async fn create_workspace_item(
    ...
    blocking(move || {
        ...
        if !valid_entry_name(&name) {
            return Err(format!("'{name}' isn't a valid file or folder name."));
        }
        if absolute.exists() {
        ...
        let api_version =
            source_api_version(&root).unwrap_or_else(|| DEFAULT_API_VERSION.to_string());
...
pub async fn rename_workspace_item(
    ...
    blocking(move || {
        ...
        if !valid_entry_name(new_name) {
            return Err(format!("'{new_name}' isn't a valid file or folder name."));
        }
...
pub async fn move_workspace_items(
    ...
    blocking(move || {
        ...
        let items = item_paths
            ...
            .map(|path| resolve_existing(&root, path))
            .collect::<Result<Vec<_>, _>>()?;
        apply_moves(&root, &move_plan(&items, &destination)?)
    })
    .await
}
...
pub async fn copy_workspace_items(
    ...
    blocking(move || {
        ...
        {
            return Err("A folder can't be copied into itself.".to_string());
        }
        apply_copies(&root, &copy_plan(&items, &destination))
    })
    .await
}
