...
use crate::commands::{
    blocking, hide_console, is_ignored_path, lock, now_millis, package_directories,
    run_with_limits, to_relative_string, workspace_root, write_atomic,
};
...
fn walk(root: &Path, dir: &Path, previous: Option<&Snapshot>, out: &mut BTreeMap<String, Stamp>) {
...
fn scan(root: &Path, previous: Option<&Snapshot>) -> BTreeMap<String, Stamp> {
    ...
    for dir in package_directories(root) {
        walk(root, &root.join(dir), previous, &mut files);
    }
    files
}
